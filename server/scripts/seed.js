...
// Use the working prisma.config.ts configuration
const prisma = new PrismaClient({}); // Config comes from prisma.config.ts
  ...
  for (const leadData of leadsData) {
    ...
    const lead = await prisma.lead.upsert({
      ...
      create: {
        ...
        doesPublicWorks: leadData.doesPublicWorks,
        unionLikely: leadData.unionLikely,
        multiJobsite: leadData.multiJobsite,
...
main()
  .then(() => console.log('Seed disconnected successfully'))
  .catch((e) => {
    console.error(e);
    process.exit(1);
  });